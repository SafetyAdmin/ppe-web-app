...
async function sendLineNotification(messageText) {
    const targetUserId = "U64847d26d65aa2158092a0de6b07ba56"; 

    console.log("🚀 กำลังส่งข้อมูลไปที่ /api/line...");
    ...
    try {
        ...
        // ✅ เช็คสถานะการตอบกลับ
        console.log(`📡 Server ตอบกลับ: ${response.status} ${response.statusText}`);
        
        // ลองอ่านข้อความที่ Server ตอบกลับมา
        const text = await response.text();
        console.log("📄 เนื้อหาที่ตอบกลับ:", text.substring(0, 100)); // ดูแค่ 100 ตัวอักษรแรก

        if (!response.ok) {
            console.error("❌ ส่งไม่ผ่าน! Server แจ้งว่า:", text);
            alert("แจ้งเตือนไลน์ไม่สำเร็จ: " + response.status);
        } else {
            console.log("✅ LINE Notification Sent (Confirmed)");
        }

    } catch (error) {
        console.error("❌ LINE Error (Network/Code):", error);
...
async function processConfirmPickup(requestCode) {
    ...
    try {
        ...
        if (request) {
            ...
            // เพิ่ม else เพื่อกันหน้าจอค้างกรณีหาไม่เจอ
